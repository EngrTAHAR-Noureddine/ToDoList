...
class AddNewTasks extends StatelessWidget {
 Task editeTask;
  ...
  void initState() {
    ...
    if((editeTask.task!=null)&&(editeTask.task.isNotEmpty))_taskName.text = editeTask.task;
    if((editeTask.timeReminder!=null)&&(editeTask.timeReminder.isNotEmpty)) _timeR = editeTask.timeReminder;

    _categorySelected =((editeTask.category!=null)&&(editeTask.category.isNotEmpty))? editeTask.category:(_categorySelected!=null && _categorySelected.isNotEmpty)?_categorySelected:"Category";

    _frequencySelected =((editeTask.frequency!=null)&&(editeTask.frequency.isNotEmpty))? editeTask.frequency:(_frequencySelected!=null && _frequencySelected.isNotEmpty)?_frequencySelected:Variables().frequency[0];

    if((editeTask.dateReminder!=null)&&(editeTask.dateReminder.isNotEmpty)){
      _part= editeTask.dateReminder.split("/");
      selectedReminder = DateTime(int.parse(_part[2]),int.parse(_part[1]),int.parse(_part[0]));
    }

    if((editeTask.date!=null)&&(editeTask.date.isNotEmpty)){
      _part= editeTask.date.split("/");
      selectedDate = DateTime(int.parse(_part[2]),int.parse(_part[1]),int.parse(_part[0]));
    }

    if((editeTask.note!=null)&&(editeTask.note.isNotEmpty)) _addNoteText.text = editeTask.note;

    _statusSelected =((editeTask.status!=null)&&(editeTask.status.isNotEmpty))? editeTask.status:Variables().status[3];

    if((editeTask.goal!=null)&&(editeTask.goal.isNotEmpty))_addGoal.text = editeTask.goal;
    if((editeTask.time!=null)&&(editeTask.time.isNotEmpty)) _time =editeTask.time ;
  }
